...
import random
from django.contrib.auth.models import User, BaseUserManager, AbstractBaseUser, send_mail, PermissionsMixin
...
class User(AbstractBaseUser, PermissionsMixin):
    ...
    email = models.EmailField(
        _('email address'), unique=False, null=True, blank=False)
    phone_number = models.BigIntegerField(unique=True, null=True, blank=True,
                                          validators=[validators.RegexValidator(
                                              r'^989[0-3,9]\d{8}$', ('Enter A valid mobile number.'), 'invalid'),],
                                          error_messages={
...
class UserProfile(models.Model):
    ...
    gender = models.BooleanField(
        _('gender'), help_text=_('male is False and female is True'), null=True)
    province = models.ForeignKey(verbose_name=_(
        'province'), to='Province', null=True, on_delete=models.CASCADE)
...
class Device(models.Model):
    ...
    type_device = models.PositiveSmallIntegerField(
        choices=DEVISE_TYPE_CHOICES, default=WEB)
